  ...
  useEffect(() => {
    if (partnerDetails) {
      setEditProfileForm({
        ...
        profile_img: partnerDetails?.partner_image, // Ensure decrypted image
      });

      // Set image for preview, if exists
      if (partnerDetails?.partner_image) {
        setUploadedImageSrc(
          `data:image/jpeg;base64,${partnerDetails?.partner_image}` // Ensure proper format
        );
      }
    }
  }, [partnerDetails]);
  ...
  const onFileInputChange = async (e) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        setEditProfileForm({ ...editProfileForm, profile_img: base64String }); // Store Base64 string in state
      };
      ...
      .post(
        `${EDIT_PROFILE}`,
        {
          params: {
            ...
            profile_img: editProfileForm.profile_img, // Send Base64
          },
        },
        {
          headers: {
          ...
          <div className="flex flex-col gap-5 items-center justify-center w-full">
            <label htmlFor="dropzone-file" className="cursor-pointer">
              <div className="flex flex-col items-center justify-center">
                {uploadedImageSrc ? (
                  <img
                    src={uploadedImageSrc} // Display the image directly
